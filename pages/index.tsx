...
const Home: NextPage = () => {
  ...
  const editionDrop = useContract("process.env.REACT_APP_EDITIONDROP_ADDRESS", "edition-drop").contract;
  // トークンコントラクトの初期化
  const token = useContract("process.env.REACT_APP_ERC20_TOKEN_ADDRESS", "token").contract;
  // 投票コントラクトの初期化
  const vote = useContract("process.env.REACT_APP_GOVERNANCE_ADDRESS_VOTE", "vote").contract;
  ...
  useEffect(() => {
    ...
    // ユーザーがメンバーシップNFTを持っているかどうかを確認する関数を定義
  ...
  // これは DAO メンバーだけが見ることができ、すべてのメンバーとすべてのトークン量をレンダリングします
    ...
    return (
      <div className={styles.container}>
        <main className={styles.main}>
        <h1 className={styles.title}>FOODAO Member Page</h1>
